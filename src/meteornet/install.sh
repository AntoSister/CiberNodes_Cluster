...
echo "*** SatCon Installer ***"

function usage() {
    printf '\nUsage: %s [-ahu]\n\n' "$(basename "$0")" >&2
    # Less options, less problems...
    echo "Options:"
    echo " -a: Install SatCon and (A)ll its dependencies. Good luck with your Internet connection!"
    echo " -h: Print usage/(H)elp."
    echo " -u: (U)pgrade installed ComNetsEmu and all its dependencies."
    exit 2
}

function ansible_install_satcon_localhost() {
    SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)
    ansible-playbook \
        --connection=local \
        --inventory 127.0.0.1, \
        --limit 127.0.0.1 "${SCRIPT_DIR}/install_satcon.yml"
}

function all() {
    ansible_install_satcon_localhost
}
